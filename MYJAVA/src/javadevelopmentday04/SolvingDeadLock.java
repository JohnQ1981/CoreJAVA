				...
				while(true) {
					...
					try {
						...
						if(flagLock1 && !doneLock1) {
							System.out.println("Inside the thread 1 and on the lock1");
							lock1.unlock();
							doneLock1 = true;
						}
			...
			public void run() {
				...
				while(true) {
					...
					try {
						...
						if(flagLock1 && !doneLock1) {
							System.out.println("Inside the thread 2 and on the lock1");
							lock1.unlock();
							doneLock1 = true;
						}
						if(flagLock2 && !doneLock2) {
							System.out.println("Inside the thread 2 and on the lock2");
							lock2.unlock();
							doneLock2 = true;
						}
						if(flagLock1 && flagLock2) {
